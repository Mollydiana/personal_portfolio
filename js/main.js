...
jQuery(window).load(function() {
    ...
    $('#apps_work_image_slider').carouFredSel({
        responsive: true,
        width: '100%',
        auto: false,
        scroll: 1,
        prev: '#apps_work_prev',
        next: '#apps_work_next',
        pagination: "#apps_work_slide_pager",
        swipe: {
            onMouse: false,
            onTouch: false
        },
	...
	/**********************************/
	jQuery('ul#portfolio-items li, ul#digital-items li, ul#apps_work_image_slider li').each(function() {
    ...
    jQuery("#apps_work_image_slider li").each(function(){
        var element_holder_width = jQuery(this).width();
        var element_holder_height = jQuery(this).height();
        var apps_work_text_width = jQuery(this).find("p.apps_work_image_slider").width();
        var apps_work_text_height = jQuery(this).find("p.apps_work_image_slider").height();
        var top = (element_holder_height - apps_work_text_height)/1.1;
        var left = (element_holder_width - apps_work_text_width)/2;
        jQuery(this).find("p.apps_work_image_slider").css({
            'top': top,
            'left': left
        });
    });
...
/*************************************************/
	jQuery('ul#portfolio-items li, ul#digital-items li, ul#apps_work_image_slider li').hover(function() {
